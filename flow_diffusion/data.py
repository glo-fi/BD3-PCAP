...
class FlowTokenizer:
    ...
    def fit(self, data: np.ndarray, feature_names: Optional[List[str]] = None):
        ...
        # Suppress sklearn warnings about bin width and constant features
        with warnings.catch_warnings():
            ...
            for i, feature_name in enumerate(feature_names):
                # Create discretizer with appropriate parameters
                discretizer_params = {
                    'n_bins': self.n_bins,
                    'encode': 'ordinal',
                ...
                # Add quantile_method for quantile strategy to suppress FutureWarning
    ...
    def detokenize(self, tokens: torch.Tensor) -> np.ndarray:
        ...
        for i, feature_name in enumerate(self.feature_names):
            ...
            out_of_range_indices = (feature_tokens.flatten() >= actual_bins) | (
                feature_tokens.flatten() < 0
            )
            ...
            if invalid_indices.any():
                ...
                feature_tokens[invalid_indices] = np.random.randint(
                    0, actual_bins, size=invalid_indices.sum()
                ).reshape(-1, 1)
            ...
            assert feature_tokens.min() >= 0, (
                f"Negative token found: {feature_tokens.min()}"
            )
            assert feature_tokens.max() < actual_bins, (
                f"Token >= actual_bins found: {feature_tokens.max()} >= {actual_bins}"
            )

            # Inverse transform (returns bin centers)
            reconstructed_feature = discretizer.inverse_transform(
                feature_tokens
            ).flatten()
...
def create_sample_flow_data(
    n_flows: int = 10000, n_features: int = 8
    ...
    for _ in range(n_flows):
        ...
        total_packets = np.random.poisson(
            max(1, duration * 5)
        )  # Packets related to duration
        avg_packet_size = np.random.normal(800, 300)  # Average packet size
        avg_packet_size = max(
            64, min(1500, avg_packet_size)
        )  # Clamp to realistic range

        total_bytes = total_packets * avg_packet_size + np.random.normal(
            0, total_packets * 50
        )
